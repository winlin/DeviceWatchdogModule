...
MITFuncRetValue recvfrom_watchdog(int fd)
{
    ...
    socket_tv.tv_sec = 1;
    socket_tv.tv_usec = 0;
    if(setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, (void *)&socket_tv, sizeof(socket_tv)) < 0) {
        MITLog_DetErrPrintf("setsockopt() timeout failed");
        reutrn MIT_RETV_FAIL;
    }
